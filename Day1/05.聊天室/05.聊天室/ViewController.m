...
@property (weak, nonatomic)
    IBOutlet NSLayoutConstraint* inputViewConstraint; //输入框的约束
@property (weak, nonatomic) IBOutlet UITableView* tableView;

@property (nonatomic, strong) NSMutableArray* chatMsgs; //聊天消息数组(数据源)
...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter]
        addObserver:self
           selector:@selector(kbFrmWillChange:)
...
- (void)reloadDataWithText:(NSString*)text
{
    ...
    NSIndexPath* lastPath =
        [NSIndexPath indexPathForRow:self.chatMsgs.count - 1
                           inSection:0];
